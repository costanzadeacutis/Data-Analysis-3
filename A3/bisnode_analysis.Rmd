...
After picking Logit X3 as the best model, I bring this model to the holdout set and I calculate RMSE on this subset of the data. The model performs on the holdout as it performs in the training set (RMSE = 0.383).
...
loss(FP)/(loss(FP) + loss(FN)), which in this case would be equal to 2/3=0.667. This would be true if the prediction model I chose, i.e. Logit X3, were the best possible prediction model. Therefore, I resort to the algorithm to find the optimal threshold via cross-validation. The resulting selected threshold for Logit X3 is 0.628, with CV expected loss 0.199, which is the lowest on average.
...
The expected loss on the holdout set when Logit X3 is applied is equal to 0.205.
...
#### Confusion table
This confusion table shows true and and missclassifications on the holdout set. It appears that the model does well in classifying correctly those firms that are non-fast-growing (99.62%), while it classifies correctly as fast-growing only 4% of the total. Given that the highest loss is registered for false positive, it is reassuring that the misclassification of non-fast growing as fast-growing is so small.
...
The model outperforms Logit X3, with a cross validated RMSE of 0.381 and AUC of 0.705.
...
The average CV expected loss is 0.199, as for the best logit (Logit X3).
...
I re-estimate the model to do predictions on the holdout set. The resulting RMSE is 0.384 and AUC is 0.701. The expected loss is 0.206, worse than the Logit X3 (0.205).
...
```
# Summary results
I can compare the Logit models, Logit LASSO, and Random Forest in predicting probabilities and classifying observations. Random forest outperforms logits and LASSO in predicting, while it performs similarly to Logit X3 in classifying. Logit X1 is the worst in predicting, while Logit LASSO has the lowest peformance in classifying. The optimal threshold are all quite close to the one suggested by the formula.
